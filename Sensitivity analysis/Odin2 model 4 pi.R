...
## Initial states:
# spring cohort
initial(M1_sp) <- prop_sp * M1_sp_ini # only a proportion of children is born protected
initial(M2_sp) <- M2_sp_ini
initial(S_sp) <- (1-prop_sp) * M1_sp_ini
initial(R_sp) <- R_sp_ini

# summer cohort
initial(M1_sm) <- prop_sm * M1_sm_ini
initial(M2_sm) <- M2_sm_ini
initial(S_sm) <- (1-prop_sm) * M1_sm_ini
initial(R_sm) <- R_sm_ini

# autumn cohort
initial(M1_au) <- prop_au * M1_au_ini
initial(M2_au) <- M2_au_ini
initial(S_au) <- (1-prop_au) * M1_au_ini
initial(R_au) <- R_au_ini

# winter cohort
initial(M1_wt) <- prop_wt * M1_wt_ini
initial(M2_wt) <- M2_wt_ini
initial(S_wt) <- (1-prop_wt) * M1_wt_ini
