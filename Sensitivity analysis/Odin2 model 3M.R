...
# spring birth cohort
n_M1M2_sp <- mu*3 * dt * M1_sp # M1 to M2
n_M2M3_sp <- mu*3 * dt * M2_sp # M2 to M3
n_M3S_sp <- mu*3 * dt * M3_sp # M3 to S
n_SR_sp <- lambda_sp * dt * S_sp # S to R

# summer birth cohort
n_M1M2_sm <- mu*3 * dt * M1_sm # M1 to M2
n_M2M3_sm <- mu*3 * dt * M2_sm # M2 to M3
n_M3S_sm <- mu*3 * dt * M3_sm # M3 to S
n_SR_sm <- lambda_sm * dt * S_sm # S to R

# autumn birth cohort
n_M1M2_au <- mu*3 * dt * M1_au # M1 to M2
n_M2M3_au <- mu*3 * dt * M2_au # M2 to M3
n_M3S_au <- mu*3 * dt * M3_au # M3 to S
n_SR_au <- lambda_au * dt * S_au # S to R

# winter birth cohort
n_M1M2_wt <- mu*3 * dt * M1_wt# M1 to M2
n_M2M3_wt <- mu*3 * dt * M2_wt # M2 to M3
n_M3S_wt <- mu*3 * dt * M3_wt # M3 to S
